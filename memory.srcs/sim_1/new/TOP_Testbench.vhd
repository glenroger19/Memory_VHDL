...
begin
    ...
    begin
        ...
        while true loop
            ...
            write(output, "LEDs: ");
            for i in led_o_tb'range loop
                write(output, std_logic'IMAGE(led_o_tb(i)));
            end loop;
            writeline(output);

            write(output, "Anode activate: ");
            for i in anode_activate_tb'range loop
                write(output, std_logic'IMAGE(anode_activate_tb(i)));
            end loop;
            writeline(output);

            write(output, "Segment output: ");
            for i in seg_o_tb'range loop
                write(output, std_logic'IMAGE(seg_o_tb(i)));
            end loop;
            writeline(output);
